...
class OwnSpacedLearnHub:
    ...
    def tutorial(self):
        message = ("Welcome in this Telegram spaced learning bot !\n"
                   "It's utility is to help you learning efficiently. For this, you'll have to learn each one "
                   "of your lesson four times, with a specific interval between each learn session.\n"
                   "Everyday, at 6am, you'll receive a message summing up what you'll have to learn : "
                   "the title of the lessons, and their ids.\n"
                   "Commands:\n"
                   "- To register a new lesson in the system, you have to send a message with this syntax:\n"
                   "J0 title_of_lesson\n"
                   "Ex: J0 Nathan Asie chap 18\n"
                   "- To mark a lesson as read, you have to send a message with this syntax:\n"
                   "id_of_lesson done\n"
                   "Ex: 186 done\n"
                   "- If you want to delete a lesson, send a message with this syntax:\n"
                   "id_of_lesson del\n"
                   "Ex: 389 del\n"
                   "- To see the lessons you have to learn today, send /today.\n"
                   "- To see all your lessons, send /list_all.\n"
                   "\n"
                   "Good luck !")
    ...
    def handle_command(self, command: str):
        ...
        if command == "/today":
            ...
            answer = ("- To mark a lesson as learnt, send its id + done.\n"
                      "Ex: 189 done\n"
                      "- To delete a lesson from database, send one of its id + del.\n"
                      "Ex: 638 del")
            return answer
            ...
            return answer
        elif command.startswith("append ") and len(command) > len("append "):
            number_of_day = command[len('append '):]
            if number_of_day.isdigit():
                number_of_day = int(number_of_day)
                self.decrease_date_to_db(number_of_day)
                return f"Decreased {number_of_day} day."
            else:
                return "Syntax: append X where X is a digit."
            ...
            if _id in list(self.planning.T.loc['id']):
                if command[1] == 'done':
                    ...
                    self.planning.at[_id, 'check'] = False
                    answer = f"Oops, undone {_id} !"
                    return answer
                elif command[1] == 'del':
    ...
    def decrease_date_to_db(self, value=1):
        for index in self.planning.index.values:
            self.planning.at[index, 'date'] -= value
        return
